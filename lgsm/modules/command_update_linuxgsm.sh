	...
	if [ -d "${modulesdir}" ]; then
		(
			...
			for modulefile in *; do
				...
				if [ "${remotereponame}" == "GitHub" ]; then
					curl --connect-timeout 10 -IsfL "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/${github_file_url_dir}/${modulefile}" 1> /dev/null
				else
				...
				if [ $? != 0 ]; then
					...
					# compare file
					if [ "${remotereponame}" == "GitHub" ]; then
						module_file_diff=$(diff "${modulesdir}/${modulefile}" <(curl --connect-timeout 10 -s "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/${github_file_url_dir}/${modulefile}"))
					else
