...
curl ${nocache} --connect-timeout 3 -IsfL "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/linuxgsm.sh" 1> /dev/null
exitcode=$?
...
if [ "${remotereponame}" == "GitHub" ]; then
	tmp_script_diff=$(diff "${tmpdir}/linuxgsm.sh" <(curl ${nocache} --connect-timeout 3 -s "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/linuxgsm.sh"))
else
...
if [ "${remotereponame}" == "GitHub" ]; then
	curl ${nocache} --connect-timeout 3 -IsfL "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/lgsm/config-default/config-lgsm/${gameservername}/_default.cfg" 1> /dev/null
else
...
if [ "${remotereponame}" == "GitHub" ]; then
	config_file_diff=$(diff "${configdirdefault}/config-lgsm/${gameservername}/_default.cfg" <(curl ${nocache} --connect-timeout 3 -s "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/lgsm/config-default/config-lgsm/${gameservername}/_default.cfg"))
else
...
if [ -f "${datadir}/${distroid}-${distroversioncsv}.csv" ]; then
	...
	if [ "${remotereponame}" == "GitHub" ]; then
		curl ${nocache} --connect-timeout 3 -IsfL "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/lgsm/data/${distroid}-${distroversioncsv}.csv" 1> /dev/null
	else
	...
	if [ "${remotereponame}" == "GitHub" ]; then
		config_file_diff=$(diff "${datadir}/${distroid}-${distroversioncsv}.csv" <(curl ${nocache} --connect-timeout 3 -s "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/lgsm/data/${distroid}-${distroversioncsv}.csv"))
	else
	...
	if [ -d "${modulesdir}" ]; then
		(
			...
			for modulefile in *; do
				...
				if [ "${remotereponame}" == "GitHub" ]; then
					curl ${nocache} --connect-timeout 3 -IsfL "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/${github_file_url_dir}/${modulefile}" 1> /dev/null
				else
				...
				if [ "${exitcode}" -ne 0 ]; then
					...
					# compare file
					if [ "${remotereponame}" == "GitHub" ]; then
						module_file_diff=$(diff "${modulesdir}/${modulefile}" <(curl ${nocache} --connect-timeout 3 -s "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/${github_file_url_dir}/${modulefile}"))
					else
