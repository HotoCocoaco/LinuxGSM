...
curl --connect-timeout 10 -IsfL "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/linuxgsm.sh" 1> /dev/null
if [ $? != "0" ]; then
...
if [ "${remotereponame}" == "GitHub" ]; then
	tmp_script_diff=$(diff "${tmpdir}/linuxgsm.sh" <(curl --connect-timeout 10 -s "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/linuxgsm.sh"))
else
...
if [ "${remotereponame}" == "GitHub" ]; then
	curl --connect-timeout 10 -IsfL "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/lgsm/config-default/config-lgsm/${gameservername}/_default.cfg" 1> /dev/null
else
...
if [ "${remotereponame}" == "GitHub" ]; then
	config_file_diff=$(diff "${configdirdefault}/config-lgsm/${gameservername}/_default.cfg" <(curl --connect-timeout 10 -s "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/lgsm/config-default/config-lgsm/${gameservername}/_default.cfg"))
else
...
if [ -f "${datadir}/${distroid}-${distroversioncsv}.csv" ]; then
	...
	if [ "${remotereponame}" == "GitHub" ]; then
		curl --connect-timeout 10 -IsfL "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/lgsm/data/${distroid}-${distroversioncsv}.csv" 1> /dev/null
	else
	...
	if [ "${remotereponame}" == "GitHub" ]; then
		config_file_diff=$(diff "${datadir}/${distroid}-${distroversioncsv}.csv" <(curl --connect-timeout 10 -s "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/lgsm/data/${distroid}-${distroversioncsv}.csv"))
	else
	...
	if [ -d "${functionsdir}" ]; then
		(
			...
			for functionfile in *; do
				...
				if [ "${remotereponame}" == "GitHub" ]; then
					curl --connect-timeout 10 -IsfL "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/${github_file_url_dir}/${functionfile}" 1> /dev/null
				else
				...
				if [ $? != 0 ]; then
					...
					# compare file
					if [ "${remotereponame}" == "GitHub" ]; then
						function_file_diff=$(diff "${functionsdir}/${functionfile}" <(curl --connect-timeout 10 -s "https://raw.gitmirror.com/${githubuser}/${githubrepo}/${githubbranch}/${github_file_url_dir}/${functionfile}"))
					else
